...
class CoefficientSet(object):
    ...
    _custom_every_boundary = False # 是否自定义每个变量的上下界
    _config_path = None
    ...
    def __init__(self, variable_names, **kwargs):
        ...
        self._custom_every_boundary = kwargs.get('customSingleBoundary')
        self._config_path = kwargs.get('configPath')
        ...
        if self._custom_every_boundary == False:
            ub = self._expand_values(value = ub)
            lb = self._expand_values(value = lb)
        # 自定义每个变量的上下界
        else:
            print('=-自定义每个变量上下界-=')
            ub = self._expand_values(value = ub, type="ub")
            lb = self._expand_values(value = lb, type="lb")
    ...
    def _expand_values(self, value, type="None"):
        ...
        if isinstance(value, np.ndarray):
            ...
            # 是否自定义每个变量上下界
            if(self._custom_every_boundary == True):
                lb_ub = pd.read_csv(os.getcwd() + '/' + self._config_path)

                if len(lb_ub['ub'])+1 != self.P:
                    raise Exception('错误：上下限约束对数 %d 与变量个数 %d 不相同，检查约束文件行数', len(lb_ub['ub']), self.P-1)

                if type == "lb":
                    variable_boundary = lb_ub['lb'].to_list()
                    # 再设置intercept的默认上下限，以免报错
                    variable_boundary.insert(0, lb_ub['lb'][0])

                elif type == "ub":
                    variable_boundary = lb_ub['ub'].to_list()
                    variable_boundary.insert(0, lb_ub['ub'][0])

                value_array = variable_boundary
